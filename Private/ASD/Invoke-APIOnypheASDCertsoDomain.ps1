	Function Invoke-APIOnypheASDCertsoDomain {
	<#
	  .SYNOPSIS
	  create input for Invoke-OnypheAPIV2 function and then call it to query the ASD Certso Domain APIv1

	  .DESCRIPTION
	  create input for Invoke-OnypheAPIV2 function and then call it to query the ASD Certso Domain APIv1 -
	  discovers certificate subject.organization value(s) linked to the given domain(s). BETA endpoint, requires
	  ...
	  .EXAMPLE
	  C:\PS> Invoke-APIOnypheASDCertsoDomain -Domain example.com
	  discovers subject.organization certificate value(s) linked to example.com
	#>
		[cmdletbinding()]
		Param (
		...
		Process {
			...
			$params = @{
				request = "v1/asd/certso/domain"
				APIInfo = "asd/certso/domain"
				APIInput = @($Domain)
				APIKeyrequired = $true
				APIVersion = "1"
