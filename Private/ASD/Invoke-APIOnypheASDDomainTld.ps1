	Function Invoke-APIOnypheASDDomainTld {
	<#
	  .SYNOPSIS
	  create input for Invoke-OnypheAPIV2 function and then call it to query the ASD Domain Tld APIv1

	  .DESCRIPTION
	  create input for Invoke-OnypheAPIV2 function and then call it to query the ASD Domain Tld APIv1 - discovers
	  related domains across different Top-Level Domains for the given domain(s). BETA endpoint, requires a
	  Griffin View or Griffin View ASM Edition subscription with a non-commercial use licence (see
	  ...
	  .EXAMPLE
	  C:\PS> Invoke-APIOnypheASDDomainTld -Domain example.com
	#>
		[cmdletbinding()]
		Param (
		...
		Process {
			...
			$params = @{
				request = "v1/asd/domain/tld"
				APIInfo = "asd/domain/tld"
				APIInput = @($Domain)
				APIKeyrequired = $true
				APIVersion = "1"
