	Function Invoke-APIOnypheASDDomainWildcard {
	<#
	  .SYNOPSIS
	  create input for Invoke-OnypheAPIV2 function and then call it to query the ASD Domain Wildcard APIv1

	  .DESCRIPTION
	  create input for Invoke-OnypheAPIV2 function and then call it to query the ASD Domain Wildcard APIv1 -
	  discovers domains associated via wildcard DNS/certificate records for the given domain(s). BETA endpoint,
	  ...
	  .EXAMPLE
	  C:\PS> Invoke-APIOnypheASDDomainWildcard -Domain example.com
	#>
		[cmdletbinding()]
		Param (
		...
		Process {
			...
			$params = @{
				request = "v1/asd/domain/wildcard"
				APIInfo = "asd/domain/wildcard"
				APIInput = @($Domain)
				APIKeyrequired = $true
				APIVersion = "1"
