	Function Invoke-APIOnypheASDDnsDomainSoa {
	<#
	  .SYNOPSIS
	  create input for Invoke-OnypheAPIV2 function and then call it to query the ASD Dns Domain Soa APIv1

	  .DESCRIPTION
	  create input for Invoke-OnypheAPIV2 function and then call it to query the ASD Dns Domain Soa APIv1 -
	  executes a live DNS SOA lookup against the given domain(s). BETA endpoint, requires a Griffin View or
	  ...
	  .EXAMPLE
	  C:\PS> Invoke-APIOnypheASDDnsDomainSoa -Domain example.com
	#>
		[cmdletbinding()]
		Param (
		...
		Process {
			...
			$params = @{
				request = "v1/asd/dns/domain/soa"
				APIInfo = "asd/dns/domain/soa"
				APIInput = @($Domain)
				APIKeyrequired = $true
				APIVersion = "1"
