	Function Invoke-APIOnypheASDCertsoWildcard {
	<#
	  .SYNOPSIS
	  create input for Invoke-OnypheAPIV2 function and then call it to query the ASD Certso Wildcard APIv1

	  .DESCRIPTION
	  create input for Invoke-OnypheAPIV2 function and then call it to query the ASD Certso Wildcard APIv1 -
	  discovers certificate subject.organization value(s) linked to the given wildcard domain pattern(s) (e.g.
	  *.example.com). BETA endpoint, requires a Griffin View or Griffin View ASM Edition subscription with a
	  ...
	  .EXAMPLE
	  C:\PS> Invoke-APIOnypheASDCertsoWildcard -Domain "*.example.com"
	#>
		[cmdletbinding()]
		Param (
		...
		Process {
			...
			$params = @{
				request = "v1/asd/certso/wildcard"
				APIInfo = "asd/certso/wildcard"
				APIInput = @($Domain)
				APIKeyrequired = $true
				APIVersion = "1"
