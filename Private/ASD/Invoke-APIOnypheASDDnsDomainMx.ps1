	Function Invoke-APIOnypheASDDnsDomainMx {
	<#
	  .SYNOPSIS
	  create input for Invoke-OnypheAPIV2 function and then call it to query the ASD Dns Domain Mx APIv1

	  .DESCRIPTION
	  create input for Invoke-OnypheAPIV2 function and then call it to query the ASD Dns Domain Mx APIv1 -
	  executes a live DNS MX lookup against the given domain(s). BETA endpoint, requires a Griffin View or
	  ...
	  .EXAMPLE
	  C:\PS> Invoke-APIOnypheASDDnsDomainMx -Domain example.com
	#>
		[cmdletbinding()]
		Param (
		...
		Process {
			...
			$params = @{
				request = "v1/asd/dns/domain/mx"
				APIInfo = "asd/dns/domain/mx"
				APIInput = @($Domain)
				APIKeyrequired = $true
				APIVersion = "1"
