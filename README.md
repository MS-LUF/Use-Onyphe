...
## Notes version (2.2.2) - Get-OnypheSummary pagination caveat (docs-only) :
 - documentation-only release, no code/behavior change. While verifying the v2.2.1 fix didn't affect other cmdlets, live-testing surfaced that `Get-OnypheSummary` (the Summary API, `v2/summary/<type>/<value>`) has the same non-pagination characteristic as the Simple API documented below - confirmed against `8.8.8.8` and `1.1.1.1` via both the cmdlet and raw HTTP: any `-Page` value returns the exact same first page of results, the server always reports `page:1` back regardless of what's requested, and an untried `size` query-string parameter has no effect either (the Summary wrappers never exposed one, consistent with that).
 - added the same NOTE-style caveat used for the Simple API fix to `Get-OnypheSummary.ps1`'s `-Page` doc block, all three `Invoke-APISummaryOnyphe<IP,Domain,Hostname>` private wrappers, `Howto.md`, `docs/Get-OnypheSummary.md`, and the two matching `<maml:para>` entries in `en-US/use-onyphe-help.xml` - pointing to `Search-OnypheInfo`/`Export-OnypheInfo` for anyone who needs more than one page of results for a category.

## Notes version (2.2.1) - Export-OnypheDiscoveryInfo silent 100-result cap fix :
 - `Export-OnypheDiscoveryInfo` (and every `Invoke-APIBulkDiscoveryOnyphe<Category>` private wrapper behind it) never exposed the Discovery bulk API's `size` query-string parameter, so Onyphe silently returned at most 100 results per OQL query line, with no error, no warning, and no truncation flag in the response - a user query that has 2789 real matches (`organization:"Bleu SAS"` against `riskscan`) came back with only 100.
 - new optional `-Size <1-10000>` parameter, threaded through `Export-OnypheDiscoveryInfo` -> `Invoke-APIBulkDiscoveryOnyphe<Category>` -> the shared `Invoke-OnypheBulkFileUpload` -> `Invoke-OnypheAPIV2`'s existing `size` support. Opt-in: omitting `-Size` keeps today's 100-result-per-line behaviour unchanged, no breaking change.
 - live-verified against the real account and query above: default (no `-Size`) returned exactly 100; `-Size 10000` returned 2791, matching the website's own count (2789, small drift from new data indexed between checks). Also confirmed live that the endpoint's `page` parameter has **no effect at all** (`?page=1` and `?page=2` return the identical 100 records) - `-Size` is the only way to get more than the default per query line, which matters most for categories like `riskscan` that have no Search/Export API equivalent (`Export-OnypheInfo` rejects them outright) and are therefore only reachable through this Discovery bulk path.
...
## Notes version (2.1.2) - Discovery API full category coverage :
 ...
 - live-verified (real Griffin View subscription, `category:resolver domain:...` queries): the Discovery bulk endpoint's `{asset}` URL segment is a literal fixed token (`v2/bulk/discovery/<category>/asset`) as previously assumed from docs alone - now directly confirmed, not inferred. Also confirmed live: Onyphe auto-prepends `category:<X>` to every query line server-side based on the URL path, so query files should **not** include a redundant `category:` prefix themselves (harmless if present - just duplicates in the `trackquery` field - but unnecessary).
...
## Notes version (2.0.1) - major internal refactor and security/quality release :
 ...
 - various documentation fixes: removed 38 stale `docs/*.md` pages left over from earlier renames/removals, fixed several copy-pasted/incorrect help blocks, fixed a stale reference to the old `.xml` config path in `Howto.md`
